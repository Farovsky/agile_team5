...
Процедура УстановитьНаКлиенте()
	...
	Иначе
		...
		Запрос = Новый ("HTTPЗапрос");
		Запрос.АдресРесурса = РазделитьURLЛкс(СсылкаНаСкачивание).ПутьКФайлуНаСервере;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияНужноДобавитьПользователя.Видимость = Метаданные.ОсновныеРоли.Количество() > 0 И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0;
...
Процедура НастроитьЭлементыФормы(ЭтаФорма)
	...
	КонецЕсли;
	Элементы.Установить.Доступность = ЗначениеЗаполнено(АктуальнаяВерсия) Или Файл.Существует();
