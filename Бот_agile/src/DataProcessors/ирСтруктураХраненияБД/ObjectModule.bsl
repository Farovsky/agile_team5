...
Процедура ВычислитьРазмерыТаблиц(Знач РазрешитьМедленныйСпособ = Ложь) Экспорт 
	...
	Иначе
		...
		Иначе
			РазмерыЗаполнены = ЗаполнитьРазмерыБазыСУБД();
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьРазмерыБазыСУБД()

	Если Не ирОбщий.ПроверитьСоединениеADOЭтойБДЛкс() Тогда
		Возврат Ложь;
	КонецЕсли; 
	ирОбщий.СостояниеЛкс("Вычисление размеров таблиц...");
	Если ирОбщий.ПараметрыСоединенияADOЭтойБДЛкс().ТипСУБД = "PostgreSQL" Тогда
		ТекстЗапроса = ПолучитьМакет("РазмерыPostgreSQL");
	Иначе
		ТекстЗапроса = ПолучитьМакет("РазмерыMSSQL");
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса.ПолучитьТекст();
