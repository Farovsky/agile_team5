			...
			Если _РежимОтладки Тогда // Можно менять на Истина в точке останова
				...
				Для Каждого ПолеТаблицыБД Из ПоляТаблицыБД Цикл
					...
					Если ПолеТаблицыБД.Метаданные <> Неопределено Тогда
						...
						КонецПопытки; 
						ПолноеИмяМД = ирОбщий.ПолноеИмяМДПоляТаблицыЛкс(ПолеТаблицыБД);
						Если ПолноеИмяМД = "" Тогда
							РольМетаданных = "Стандартный";
						Иначе
							Фрагменты = ирОбщий.СтрРазделитьЛкс(ПолноеИмяМД);
							РольМетаданных = Фрагменты[Фрагменты.Вграница() - 1];
						КонецЕсли;
						СтрокаКолонкиБД.РольМетаданных = РольМетаданных;
					КонецЕсли; 
