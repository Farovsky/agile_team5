...
//  Не используется.
//
Функция ИсследоватьОбъект(Объект, Модально = Истина, пПутьКДанным = Неопределено, ИмяТекущегоСвойства = Неопределено, ПрикрепитьОкно = Ложь) Экспорт 
	
	Если ПрикрепитьОкно И Не ирОбщий.ЛиЕстьМодальныеГруппыЛкс() Тогда
		КлючУникальности = "Связанный";
	Иначе
		КлючУникальности = Объект;
	КонецЕсли;
	ФормаИсследователя = ирОбщий.ПолучитьФормуЛкс(Метаданные().ПолноеИмя() + ".Форма.ИсследовательОбъектов", , , КлючУникальности);
