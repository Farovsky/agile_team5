...
Процедура Конструктор(Объект) Экспорт 
	...
	Исключение
		// Элемент не выбран! если записан в отменной транзакции https://www.hostedredmine.com/issues/948125
		Модифицированность = Ложь;
		СсылкаНового = Неопределено;
	КонецПопытки;
	Если Объект.Ссылка.Пустая() Тогда
		ЭтотОбъект._СсылкаНового = СсылкаНового;
	Иначе
		ЭтотОбъект._СсылкаНового = Ссылка;
	КонецЕсли; 
	...
	Если ирКэш.РежимОтладкиЛкс() Тогда 
		...
		Для Каждого СтрокаПоля Из ПоляТаблицыБД Цикл
			...
			Если СтрокаПоля.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда
				ТаблицаТЧ = Объект[СтрокаПоля.Имя].Выгрузить();
				...
				Для Каждого КолонкаТаблицы Из ТаблицаТЧ.Колонки Цикл
					КолонкаТаблицы.Ширина = 0;
				КонецЦикла;
				Данные.Вставить(СтрокаПоля.Имя, ТаблицаТЧ);
			Иначе
...
Функция ОбъектБД(ВосстановитьДанные = Истина) Экспорт 
	...
	//Иначе
		...
		Результат = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицы, КлючОбъекта,, Ложь, Ложь,,,, Не ВосстановитьДанные).Данные;
	//КонецЕсли; 
	...
	Если ВосстановитьДанные Тогда
		...
		КонецЕсли; 
		Если ИменаРеквизитов = Неопределено И Данные.Свойство("ЭтоГруппа") Тогда
			ИменаРеквизитов = ирОбщий.СтрСоединитьЛкс(ирОбщий.РеквизитыОбъектаБДЛкс(Результат).ВыгрузитьЗначения(), ",");
		КонецЕсли; 
		ЗаполнитьЗначенияСвойств(Результат, Данные, ИменаРеквизитов); 
