	...
	Если Типы.Типы().Количество() = 1 Тогда
		...
		Иначе
			...
			Если СоздатьОбъект И Не ирОбщий.ЛиТипТаблицыМетассылкиЛкс(Тип) Тогда
				...
				Форма = ирОбщий.ОткрытьОбъектВРедактореОбъектаБДЛкс(СтруктураОбъекта,,, Истина);
				Значение = Форма.ВыбОбъект;
			Иначе
				Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(Значение) Тогда
					ТекущаяСтрока = Значение;
				КонецЕсли;
				Значение = ирОбщий.ОткрытьФормуСпискаЛкс(Тип, Отбор,,, Истина,, ТекущаяСтрока, Истина);
			КонецЕсли;
...
Процедура ВыбратьЗначениеВЯчейке(СоздатьОбъект = Ложь) Экспорт
	...
	КонецЕсли;
	ВычислитьЗависимостиКонвертации();
	ТекущаяСтрока = КонвертироватьСтрокуТД(ТабличныйДокумент, ТекущаяОбласть.Верх);
	...
	Иначе
		...
		Для НомерСтроки = НачальнаяСтрока По КонечнаяСтрока Цикл
			Если ирОбщий.ОбработатьИндикаторЛкс(Индикатор) Тогда 
				...
				КонецЕсли;
			КонецЕсли; 
			ТекущаяСтрока = КонвертироватьСтрокуТД(ТабличныйДокумент, НомерСтроки, , КоличествоОшибокКонвертации, Истина, КэшПоиска);
			#Если Сервер И Не Сервер Тогда
...
Функция СтруктураПозицийТД(ОбязательноЗаголовки = Ложь, СообщитьДинамическиеКоординаты = Ложь) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ПерваяСтрокаДанныхТД) Тогда
		...
		Для НомерСтроки = Макс(1, СтрокаЗаголовковТД) По Мин(СтрокаЗаголовковТД + 10, ТабличныйДокумент.ВысотаТаблицы) Цикл
			//КоличествоЯчеек = 0;
	...
	Если Не ЗначениеЗаполнено(ПоследняяСтрокаДанныхТД) Тогда
		...
		Для НомерСтроки = Результат.НомерСтрокиДанных По ТабличныйДокумент.ВысотаТаблицы Цикл
			КоличествоЯчеекЗаполнено = 0;
			Для НомерКолонки = 1 По Мин(20, ТабличныйДокумент.ШиринаТаблицы) Цикл
				ОбластьЦ = ТабличныйДокумент.Область(НомерСтроки, НомерКолонки);
				Если ЗначениеЗаполнено(ОбластьЦ.Текст) Тогда
		...
		Если СообщитьДинамическиеКоординаты Тогда
			ирОбщий.СообщитьЛкс("Автоподобром найдено заголовки на строке №" + Результат.НомерСтрокиЗаголовков + " и данные в строках [" + Результат.НомерСтрокиДанных + ";" + Результат.НомерПоследнейСтрокиДанных + "]");
		КонецЕсли;
	КонецЕсли;
...
Функция ЗагрузитьВБДИзТаблицыЗначений(Параметры) Экспорт
	...
	Если СвязиИПараметрыВыбора Тогда
		...
		Если Не ирОбщий.ПроверитьТаблицуЗначенийНаСогласованностьЛкс(ТаблицаБД, КолонкиСПроверками, КолонкиСНарушениями,,,, ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицыБД)) Тогда 
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция КонвертироватьСтрокуТД(ТабличныйДокумент, НомерСтроки, ТекстыЯчеек = Неопределено, КоличествоОшибок = 0, Знач РазрешитьСозданиеОбъектов = Ложь, КэшПоиска = Неопределено)
	...
	Для Этап = 1 По 2 Цикл
		Для каждого СтрокаСопоставления Из ПомеченныеКолонки Цикл
			...
			Тогда
				Примечание = ОбработатьОбласть(ОбластьЯчейки, СтрокаСопоставления, РезультатыЯчеек, РазрешитьСозданиеОбъектов, КэшПоиска, ТекстЯчейки, РезультатЯчейки);
				Если ДобавлятьСырыеДанныеВТЗ Тогда
			...
			Если Истина
				...
				И (Ложь
					Или ОбластьЯчейки.СодержитЗначение 
			...
			Тогда
				...
				ОбластьЯчейки.СодержитЗначение = ЗначениеЗаполнено(Примечание);
			КонецЕсли; 
		КонецЦикла;
	КонецЦикла;
...
//  ТекстыЯчеек    - массив текстов ячеек строки
//
Функция ОбработатьОбласть(Знач Область, СтрокаСопоставления, ТекущиеДанные, РазрешитьСозданиеОбъектов, КэшПоиска, Представление, Результат)
	...
	Тогда 
		...
		Если Результат <> СтрокаСопоставления.ОписаниеТипов.ПривестиЗначение(Результат) Тогда
			Примечание = "Расшифровка ячейки вне границ типа";
		КонецЕсли;
	ИначеЕсли ПустаяСтрока(Представление) Тогда
		Результат = Неопределено;
	Иначе
		НайденныеЗначения = ПолучитьВозможныеЗначения(СтрокаСопоставления, Представление, Примечание, ТекущиеДанные, РазрешитьСозданиеОбъектов, КэшПоиска);
		...
		Иначе
			...
			Если Не СтрокаСопоставления.ОписаниеТипов.СодержитТип(ТипЗнч(Расшифровка)) Тогда
				Область.Расшифровка = Новый СписокЗначений;
				Область.Расшифровка.Добавить(НайденноеЗначение);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
				...
				ИначеЕсли ОбъектМД <> Неопределено Тогда
					...
					Иначе
						...
						Если ПустаяСтрока(ИскатьПо) Или ПоляТаблицыБД.Найти(ИскатьПо) = Неопределено Тогда
							СтрокаОсновногоПредставления = Строка(ОбъектМД.ОсновноеПредставление);
						...
						Тогда
							...
							Если Истина
								...
								И ПоляТаблицыБД.Найти("Владелец").ТипЗначения.СодержитТип(ТипЗнч(СвязьПоВладельцу))
							Тогда
								СтруктураОтбора.Вставить("Владелец", СвязьПоВладельцу);
							КонецЕсли;
						КонецЕсли;
						КлючКэша = ОбъектМД.ПолноеИмя() + "." + ирОбщий.ОбъектВСтрокуJSONЛкс(СтруктураОтбора);
						...
						Если НайденныеЗначения = Неопределено Тогда
							...
							Для Каждого КлючИЗначение Из СтруктураОтбора Цикл
								...
								|	_Таблица." + ИскатьПо + " = &" + КлючИЗначение.Ключ;
							КонецЦикла;
							Выборка = Запрос.Выполнить().Выбрать();
							...
							Тогда
								НовыйОбъект = ирОбщий.ОбъектБДПоКлючуЛкс(ОбъектМД.ПолноеИмя());
								...
								Если ЗначениеЗаполнено(НовыйОбъект.Методы.Ссылка) Тогда
									...
									ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Создан элемент ""%1"" в справочнике ""%2"" для колонки ""%3""", Представление, ОбъектМД.Имя, ОписаниеКолонки.ИмяКолонкиПриемника));
								КонецЕсли;
							КонецЕсли;
...
Функция мПолучитьНастройку(ДляФайла = Ложь) Экспорт
	...
	ДокументРезультат.Вставить("ДополнениеЗапросом", мДополнениеЗапросом);
...
Процедура мЗагрузитьНастройку(Настройка, ЗаменитьДанныеТДиТЗ = Истина, ЗаменитьМетаданныеТДиТЗ = Истина) Экспорт
	...
	Иначе
		...
		КонецЕсли; 
	КонецЕсли; 
	СопоставлениеКолонокТЗЗаполнить(Настройка <> Неопределено);
	СопоставлениеКолонокБДЗаполнитьПоИменамСинонимам(Настройка <> Неопределено);
...
Процедура СопоставлениеКолонокБДЗаполнитьПоИменамСинонимам(РазрешитьМенятьИзИсточника = Истина, Знач СообщитьОПометке = Ложь) Экспорт
	...
	Для Этап = 1 По 2 Цикл
		Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
			...
			Для Каждого СтрокаСопоставления Из СтрокиСопоставления Цикл
				...
				Тогда
					СопоставитьКолонкуБД(СтрокаСопоставления, Колонка, РазрешитьМенятьИзИсточника, СообщитьОПометке);
...
Процедура СопоставитьКолонкуБД(Знач СтрокаСопоставления, Знач КолонкаИсточника = Неопределено, Знач РазрешитьМенятьПометку = Истина, Знач СообщитьОПометке = Ложь) Экспорт 
	
	Если Ложь
		Или РазрешитьМенятьПометку
		Или (Истина
	...
	Тогда
		...
		ИначеЕсли Не СтараяПометка И СтрокаСопоставления.ИзИсточника Тогда
...
Процедура СопоставитьКолонкуТЗ(Знач СтрокаСопоставления, Знач Колонка = Неопределено, УстановитьДопСвойства = Истина) Экспорт 
	...
	Иначе
		Если СтрокаСопоставления.РежимКонвертации = "ИзИсточника" Тогда
			СтрокаСопоставления.РежимКонвертации = "Устанавливать";
		КонецЕсли; 
		СтрокаСопоставления.ИмяКолонкиИсточника = "";
		СтрокаСопоставления.СинонимКолонкиИсточника = "";
...
Процедура ОбновитьСопоставлениеКолонокТЗ(УстановитьПометкуДляСопоставленных = Ложь, УстановитьПометкуДляНовых = Истина, ТолькоВыделенныеСтрокиТД = Ложь) Экспорт 
	...
	//КолонкиТабличногоДокумента = ПолучитьКолонкиТабличногоДокумента();
	Для Каждого СтрокаКолонкиТЗ Из СопоставлениеКолонокТЗ Цикл
		//Если Не СтрокаКолонкиТЗ.РежимКонвертации = "ИзИсточника" Тогда
		//	Продолжить;
		//КонецЕсли; 
