...
Перем мИмяПоляКоличествоДвиженийСкрытое Экспорт; // НеИзменяется
...
Функция ВыполнитьЗаданиеГрупповойОбработки(Знач ФормаОбработки, Знач Кнопка = Неопределено, Знач БлокируемыеЭлементыФормы = Неопределено, Знач НастройкаВыполнения = Неопределено, РазрешитьМногопоточность = Истина) Экспорт 
	...
	ПараметрыЗадания.Вставить("ИмяОбработки", ирОбщий.ПоследнийФрагментЛкс(ирОбщий.ИмяФормыИзФормыЛкс(ФормаОбработки)));
	...
	РезультатЗадания = ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьГрупповуюОбработку", ПараметрыЗадания, ФормаОбработки, "ОбработкаОбъектов", "Обработка объектов " + ПараметрыЗадания.ИмяОбработки,
		Кнопка, "ВыполнитьОбработкуЗавершение",, БлокируемыеЭлементыФормы, Истина,, Истина, ПрефиксыОповещений);
...
Процедура ВернутьПараметрыПослеОбработки(РеквизитыОбработки, ВладелецФормы) Экспорт
	...
	#КонецЕсли
	ирОбщий.ОповеститьОЗаписиОбъектаЛкс(ОписаниеТипов.Типы(), ВладелецФормы);
	...
	Если Не Отказ Тогда
		...
		Попытка
			...
			Если ДинамическаяВыборка Тогда
				...
				Иначе
					...
					Пока ВыборкаКлючей.Следующий() Цикл
						...
						ДобавитьОбъектВОчередьОбработки(ТипТаблицы, СтруктураКлючаОбъектаДоп, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, КопияСтрокОбъекта, ПараметрыОбработки, ТранзакцииРазрешены,
							ИндексКлюча = 0, ЭтоПоследнийОбъектБД, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта, КопияСтрокОбъекта, СтруктураПотоков);
			...
			Если КоличествоСтрок > 0 Тогда
				...
				ирОбщий.СообщитьЛкс("*Строк обработано - " + КоличествоСтрок + ", неуспешно - " + мКоличествоСтрокНеуспешно + ", успешно - " + мКоличествоСтрокУспешно + ". "
...
мИмяПоляКоличествоДвиженийСкрытое = "_КоличествоДвиженийСкрытое" + ирОбщий.СуффиксСлужебногоСвойстваЛкс();
