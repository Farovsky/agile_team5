...
Процедура ДобавитьПараметрыИзЗапроса(Знач пЗапросОтладки, пСтрокаЗапроса)
	...
	Если пЗапросОтладки.Параметры.Свойство("_ОбъектыДанныхТранзакции") Тогда
		пСтрокаЗапроса.КодПередВыполнениемЗапроса = "НачатьТранзакцию(); 
...
Процедура ЗаполнитьПараметрыADOДляЭтойБД(Знач ИсточникДанных, Автоподключение = Ложь) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ПараметрыСоединения.ИмяБД) Тогда
		Если ирОбщий.ОткрытьФормуСоединенияСУБДЛкс() = Неопределено Тогда 
			Возврат;
		КонецЕсли;
		ПараметрыСоединения = ирОбщий.ПараметрыСоединенияADOЭтойБДЛкс();
	КонецЕсли;
	Если ПараметрыСоединения.ТипСУБД = "MSSQL" Тогда
...
Функция ConnectADO(ИсточникДанных, выхСоединениеADO = Неопределено, стОпции = Неопределено, Еррорс = Неопределено, выхСтрокаСоединенияДляСообщения = "", Асинхронно = Ложь, выхСоединениеФакт = "") Экспорт
	...
	Исключение
		...
		|	" + ОписаниеОшибки;
		Если Найти(ОписаниеОшибки, "ADODB.Connection") > 0 Тогда 
			Если ИсточникДанных.Платформа = 18 Тогда // ADO-PostgreSQL
				СтрЕррор = СтрЕррор + "
				|	Используйте 32-разрядное приложение и установите от имени администратора через ""regsvr32 PGOLEDB.DLL"" 32-разрядный поставщик отсюда https://ftp.postgresql.org/pub/projects/pgFoundry/oledb/PgOleDb/1.0.0.20/PgOleDb-1.0.0.20.zip";
			КонецЕсли;
		КонецЕсли;
		СтрЕррор = СтрЕррор + "
